...
**Path rules**: Always use the full literal path in all Bash commands — NOT `~` or `$HOME` (both trigger "shell expansion syntax" approval prompts). Resolve the home directory once and use it everywhere. When constructing subagent prompts, replace `{SKILL_DIR}` with the full literal path (typically `/home/user/radiocom-36076548/.claude/skills/event-prospecting`).

**Output directory**: All event prospecting output goes to `<project>/gtm-reports/{event_slug}_prospects_{YYYY-MM-DD-HHMM}/`. Final deliverable is `index.html` (people grouped by company, ranked by company ICP), with `companies.html` and `people.html` (filterable) as alternate views, plus `results.csv` for cold-outbound import.
...
OUTPUT_DIR="$PWD/gtm-reports/${EVENT_SLUG}_prospects_${TIMESTAMP}"
...
- `{SKILL_DIR}` → full literal skill path (e.g. `/home/user/radiocom-36076548/.claude/skills/event-prospecting`)
