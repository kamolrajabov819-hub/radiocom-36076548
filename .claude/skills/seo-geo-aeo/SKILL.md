...
## Local setup (added for this repo)

Upstream this skill was written inside one author's Claude sandbox and hardcoded that
sandbox's absolute paths. Vendored here as a project skill, the paths are repo-relative:

- **Reports** are written to `seo-reports/` in the project root. Create it first
  (`mkdir -p seo-reports`); it is gitignored, so reports stay out of commits.
- **`$DOCX_SKILL`** is the root of whichever `docx` skill is available on this machine.
  Resolve it once before using it, and skip validation if nothing is found:
  ```bash
  DOCX_SKILL=$(ls -d ~/.claude/skills/synced/docx ~/.claude/skills/docx \
                     .claude/skills/docx 2>/dev/null | head -1)
  ```
- **PDF conversion** needs LibreOffice. If `$DOCX_SKILL` is empty but `soffice` is on
  `PATH`, convert directly instead:
  ```bash
  soffice --headless --convert-to pdf --outdir seo-reports/ seo-reports/<file>.docx
  ```
- **The Node `docx` package** is not a dependency of this project. Install it on demand
  outside the project tree (`npm i -g docx`, or `npx --package=docx node build.js`) —
  do not add it to `package.json`.

There is overlap with the installed `seo-geo` skill. Prefer `seo-geo` when the result
feeds `seo-audit`'s fan-out; prefer this one for a standalone, client-facing scored report.
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync('seo-reports/seo-audit-[domain]-[date].docx', buffer);
...
```bash
python "$DOCX_SKILL/scripts/office/validate.py" seo-reports/seo-audit-[domain]-[date].docx
```
...
```bash
python "$DOCX_SKILL/scripts/office/soffice.py" --headless --convert-to pdf seo-reports/seo-audit-[domain]-[date].docx --outdir seo-reports/
```

### Deliver to the user

Surface both files with `present_files` if it is available; otherwise just give the user the
paths, relative to the project root:

```
Your audit report is ready:
[Download Word Doc](seo-reports/seo-audit-[domain]-[date].docx)
[Download PDF](seo-reports/seo-audit-[domain]-[date].pdf)
```
