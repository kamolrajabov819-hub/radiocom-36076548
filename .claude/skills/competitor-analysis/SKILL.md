...
**Output directory**: All output goes to `<project>/gtm-reports/{company_slug}_competitors_{YYYY-MM-DD}/`. This directory contains one `.md` file per competitor plus the generated HTML views and CSV.
...
```bash
OUTPUT_DIR="$PWD/gtm-reports/{company_slug}_competitors_{YYYY-MM-DD}"
mkdir -p "$OUTPUT_DIR"
```
...
- **Opened in browser**: <project>/gtm-reports/{company_slug}_competitors_{date}/index.html
```
